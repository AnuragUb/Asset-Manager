...
async function main() {
  ...
  try {
    ...
    await client.query(`
      DELETE FROM asset_it_details aid
      WHERE NOT EXISTS (
        SELECT 1 FROM assets a WHERE a.id = aid.assetid
      )
    `);

    await client.query(`
      DELETE FROM project_assets pa
      WHERE NOT EXISTS (
        SELECT 1 FROM projects p WHERE p.id = pa.projectid
      )
      OR NOT EXISTS (
        SELECT 1 FROM assets a WHERE a.id = pa.assetid
      )
    `);

    await client.query(`
      DELETE FROM project_order_items poi
      WHERE EXISTS (
        SELECT 1
        FROM project_orders po
        WHERE po.id = poi.orderid
          AND NOT EXISTS (
            SELECT 1 FROM projects p WHERE p.id = po.projectid
          )
      )
    `);

    await client.query(`
      DELETE FROM project_orders po
      WHERE NOT EXISTS (
        SELECT 1 FROM projects p WHERE p.id = po.projectid
      )
    `);
    ...
    await client.query(`
      ...
      WHERE assetid IS NOT NULL
        AND BTRIM(assetid) <> ''
        AND NOT EXISTS (
          SELECT 1 FROM assets a WHERE a.id = poi.assetid
        )
    `);

    await client.query(`
      DELETE FROM asset_hierarchy h
      WHERE (
        parentid IS NOT NULL
        AND BTRIM(parentid) <> ''
        AND NOT EXISTS (
          SELECT 1 FROM assets a WHERE a.id = h.parentid
        )
      )
      OR (
        assetid IS NOT NULL
        AND BTRIM(assetid) <> ''
        AND NOT EXISTS (
          SELECT 1 FROM assets a WHERE a.id = h.assetid
        )
      )
    `);

    await client.query(`
      DELETE FROM layout_markers lm
      WHERE assetid IS NOT NULL
        AND BTRIM(assetid) <> ''
        AND NOT EXISTS (
          SELECT 1 FROM assets a WHERE a.id = lm.assetid
        )
    `);
    ...
    await client.query(`CREATE INDEX IF NOT EXISTS idx_layout_markers_assetid ON layout_markers(assetid)`);
    ...
    const statements = [
      ...
      {
        name: 'fk_layout_markers_assetid_assets',
        sql: `ALTER TABLE layout_markers ADD CONSTRAINT fk_layout_markers_assetid_assets FOREIGN KEY (assetid) REFERENCES assets(id) ON DELETE CASCADE`,
      },
      {
    ...
    console.log('Local 8080 safe parity alignment completed.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Local 8080 safe parity alignment failed:', error);
    process.exit(1);
  } finally {
    await client.end();
  }
